...
class WanderBehaviour(Behaviour):
    ...
    def do(self, world: World, elapsed: float, location: Location, position: Position, ant: BehaviourAnt):
        try:
            ...
            self.change_dir_random_chance(world,elapsed,location,position,self.variation_chance,self.wobble_chance, ant)
        except IndexError:
    ...
    def change_dir_random_chance(self, world: World, elapsed: float, location: Location, pos: Position, variation_chance:float, wobble_chance:float, ant:BehaviourAnt):
        ...
        def change_dir_random(world: World, elapsed: float, location: Location, pos: Position):
            ...
            if len(possible_free_locations) > 0:
                ...
                WanderBehaviour.move(ant, world.get_location(pos.x,pos.y), world.get_location(random_available_space[0],random_available_space[1]))
        ...
        if  variation_chance < random.random(): #90% of the time
            ...
            if world.get_location(pos.get_coordinates()[0] + ant.current_dir[0], pos.get_coordinates()[1] + ant.current_dir[1]).is_free() and random.random() < wobble_chance:
                WanderBehaviour.move(ant, world.get_location(pos.x,pos.y), world.get_location(pos.get_coordinates()[0] + ant.current_dir[0], pos.get_coordinates()[1] + ant.current_dir[1]))
            else:
                ...
                if world.get_location(pos.get_coordinates()[0] + next_dirr[0][0],pos.get_coordinates()[1] + next_dirr[0][1]).is_free():
                    WanderBehaviour.move(ant, world.get_location(pos.x,pos.y), world.get_location(pos.get_coordinates()[0] + next_dirr[0][0], pos.get_coordinates()[1] + next_dirr[0][1]))
                ...
                # else if the second picked adjacent spot is free go there
                    ...
                    WanderBehaviour.move(ant, world.get_location(pos.x,pos.y), world.get_location(pos.get_coordinates()[0] + next_dirr[1][0], pos.get_coordinates()[1] + next_dirr[1][1]))
